...
class DateRangeSelection {
  ...
  factory DateRangeSelection.resolve(
    ...
    switch (preset) {
      ...
      case DateRangePreset.previousMonth:
        ...
        return DateRangeSelection(
          preset: preset,
          from: from,
          to: DateTime(date.year, date.month, 0),
        );
      ...
      case DateRangePreset.previousYear:
        return DateRangeSelection(
          ...
          to: DateTime(date.year - 1, 12, 31),
        );
      case DateRangePreset.custom:
        final from = _date(customFrom ?? date);
        final to = _date(customTo ?? from);
        if (to.isBefore(from))
          throw ArgumentError('نهاية الفترة يجب أن تكون بعد بدايتها');
        ...
        child: Wrap(
          children: [
            for (final item in DateRangePreset.values)
              ListTile(
                title: Text(DateRangeSelection.resolve(item).label),
