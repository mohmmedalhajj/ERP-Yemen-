    ...
    await _database.transaction((txn) async {
      for (final row in lowStock) {
        ...
        await txn.insert('local_notifications', {
          ...
          'title': 'مخزون منخفض',
          'body':
              '${row['name_ar']} في ${row['warehouse_name']} — المتاح ${row['quantity_minor']}، الحد ${row['reorder_point_minor']}',
      ...
      for (final row in expiring) {
        ...
        await txn.insert('local_notifications', {
          ...
          'body':
              '${row['name_ar']} — الدفعة ${row['batch_no']} — الصلاحية ${row['expiry_date']} — الكمية ${row['quantity_minor']}',
          'scheduled_at': now.toIso8601String(),
